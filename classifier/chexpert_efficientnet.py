...
NUMBER_OF_CLASSES = 1
CHECKPOINT_DIR = f'./saved_models/chexpert_efficientnet_imbalanced_pe'
os.makedirs(CHECKPOINT_DIR, exist_ok=True)
TRAIN_SPLIT = './data/chexpert/imbalanced_pe_sd/chexpert_train_imbalanced.csv'
VAL_SPLIT = './data/chexpert/imbalanced_pe_sd/chexpert_val_imbalanced.csv'
...
class CheXpertDataset(Dataset):
    def __init__(self, csv_file, transform=None):
        ...
        required_columns = ['Path','Sex','Age','Frontal/Lateral','AP/PA','No Finding','Cardiomegaly','Pleural Effusion','Support Devices']
        data = data[required_columns]
        
        for column in ['Pleural Effusion', 'Support Devices']:
            print(f'Original {column} distribution:')
        ...
        self.labels = torch.tensor(self.data.iloc[:, 7:].values.astype(np.float32))
...
def main_worker(rank, world_size):
    ...
    # Create datasets
    train_dataset = CheXpertDataset(TRAIN_SPLIT, 
                                  transform=train_transform)
    val_dataset = CheXpertDataset(VAL_SPLIT, 
    ...
    train_loader = DataLoader(
        train_dataset, 
        batch_size=64,  # Reduced batch size per GPU
    ...
    model = CheXpertClassifier(num_classes=NUMBER_OF_CLASSES).to(rank)
