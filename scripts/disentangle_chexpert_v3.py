...
def run_inference_on_gpu(gpu_id, config, prompt_neutral, style_prompt_dict, seeds, lambda_t_stars, total_tasks):
        ...
        for seed in seeds:
            for style, prompt_style in style_prompt_dict.items():
                for lambda_t_star in lambda_t_stars:
                    save_dir = f'outputs/chexpert-swap-normal/{style}/seed{seed}/lambda_t_star{lambda_t_star}'
...
if __name__ == "__main__":
    ...
    config = InferenceConfig(
        ...
        checkpoint_path='../saved_models/finetuned_chexpert_19Feb/checkpoint-40-0/model.safetensors',
        device='cuda',
    )
    ...
    # Define seeds and lambda_t_stars
    seeds = list(range(1500, 2500))
    ...
    with Pool(num_gpus) as pool:
        pool.starmap(
            ...
            [(gpu_id, config, prompt_neutral, style_prompt_dict, seeds_split[gpu_id], lambda_t_stars, total_tasks) for gpu_id in range(num_gpus)]
        )
