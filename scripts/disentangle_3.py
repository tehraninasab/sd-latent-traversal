...
class VGGPerceptualLoss(torch.nn.Module):
    ...
    def forward(self, input, target, feature_layers=[0, 1, 2, 3], style_layers=[]):
        print(input.device, self.mean.device, self.std.device)
...
class StableDiffusionInference:
    def __init__(self, config: InferenceConfig):
        ...
        if config.checkpoint_path:
            ...
            self.model.unet.load_state_dict(checkpoint['state_dict'])
        ...
        # self.model.enable_model_cpu_offload()  # Disable CPU offloading for training
    ...
    def optimize(self, prompt_neutral: str, prompt_style: str) -> torch.Tensor:
        lambda_t_star = 12
        ...
        for epoch in range(10):
            ...
            torchvision.utils.save_image(original_image[0], f"outputs/test_optmization/original_confused.png")
            torchvision.utils.save_image(generated_image[0], f"outputs/test_optmization/generated_angry_{epoch}.png")   
            #propogate loss 
            optimizer.zero_grad()
...
if __name__ == "__main__":
    ...
    prompt_neutral = "a photo of a girl"
    prompt_style = "a photo of a girl, confused"
    image, weighting_parameter = sd_inference.optimize(prompt_neutral, prompt_style)
