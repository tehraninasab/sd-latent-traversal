...
file_handler = logging.FileHandler(f'out/finetune_isic2019_{timestamp}.log')
...
class ISIC2019Dataset(Dataset):
    ...
    def create_captions(self):
        """
        Create descriptive captions for ISIC skin lesion images based on diagnostic classes
        and visual characteristics including artifacts and lesion features.
        ...
        artifact_columns = ['dark_corner', 'hair', 'gel_border', 'gel_bubble', 
                        'ruler', 'ink', 'patches']
        ...
        for _, row in self.df.iterrows():
            ...
            for disease in disease_mapping:
                if disease in row and row[disease] == 1:
                    ...
                    caption_parts.append(f"a dermoscopic image with {full_name} ({disease})")
                    break
            ...
            # Add artifacts information if present
            artifacts_present = []
            for artifact in artifact_columns:
                if artifact in row and row[artifact] == 1:
                    readable_artifact = artifact.replace('_', ' ')
                    artifacts_present.append(readable_artifact)
            
            if artifacts_present:
                artifacts_text = ", ".join(artifacts_present)
                caption_parts.append(f"showing {artifacts_text}")
            else:
                caption_parts.append("with no visible artifacts")
    ...
    def __getitem__(self, idx):
        ...
        #if the file is not found, try with the name _downsampled
        ...
        #save the image
        #image.save(f'/home/amarkr/dent/outputs/temp/{image_name}')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fine-tune Stable Diffusion on ISIC2019 dataset with artifacts")
    parser.add_argument("--model_name_or_path", type=str, default="runwayml/stable-diffusion-v1-5")
    if socket.gethostname()=='kronos':
        parser.add_argument("--train_data_path", type=str, default='/usr/local/data/amarkr/dent/data/isic2019/isic2019_train.csv')  # Updated path
        parser.add_argument("--image_root_path", type=str, default='/usr/local/faststorage/datasets/')
        parser.add_argument("--output_dir", type=str, default='/usr/local/data/amarkr/dent/saved_models/isic2019_finetune_sd1.5')  # Updated output directory
    else:
        parser.add_argument("--train_data_path", type=str, default='data/isic2019/isic2019_train.csv')
        parser.add_argument("--image_root_path", type=str, default='/home/jupyter/ISIC_2019_Training_Input/orig')
        parser.add_argument("--output_dir", type=str, default='saved_models/isic2019_finetune_sd1.5')
