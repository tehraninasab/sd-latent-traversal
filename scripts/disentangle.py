...
class StableDiffusionInference:
    ...
    def optimize(self, prompt_neutral: str, prompt_style: str, save_dir: str) -> torch.Tensor:
        lambda_t_star = self.config.lambda_t_star
        ...
        os.makedirs(f"outputs/chexpert/seed{config.seed}_lambda_t_star{lambda_t_star}", exist_ok=True)
    ...
    def swap_embeddings(self, prompt_neutral: str, prompt_style: str, save_dir: str):
        def swap_callback(pipe, step_index, timestep, callback_kwargs):
            num_images_per_prompt = 1

            if step_index == self.config.lambda_t_star:  # Change embedding after step 10
                prompt_embeds, negative_prompt_embeds = self.model.encode_prompt(
                    prompt=prompt_style, 
                    device=self.config.device, 
                    num_images_per_prompt=num_images_per_prompt, 
        ...
        original_latent = self.model(prompt=prompt_neutral, latents=fixed_noise, output_type='latent').images
...
if __name__ == "__main__":
    ...
    prompt_style = "Chest X-ray showing Support Devices"
    neutral = "normal"
    style = "support_devices"
    # save_dir = f'outputs/chexpert/{neutral}-{style}-optimize/seed{config.seed}_lambda_t_star{config.lambda_t_star}'
    # os.makedirs(save_dir, exist_ok=True)
    # image, weighting_parameter = sd_inference.optimize(prompt_neutral, prompt_style, save_dir)
    
    for seed in [10, 15, 20, 25, 30, 35, 40, 45, 50]:
        for lambda_t_star in [10, 20, 30, 40, 50]:
            config.seed = seed
            config.lambda_t_star = lambda_t_star
            
            save_dir = f'outputs/chexpert/{neutral}-{style}-swap/seed{config.seed}_lambda_t_star{config.lambda_t_star}'
            os.makedirs(save_dir, exist_ok=True)
            image = sd_inference.swap_embeddings(prompt_neutral, prompt_style, save_dir)
            break
        break
